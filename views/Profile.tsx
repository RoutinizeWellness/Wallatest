...
export const Profile: React.FC<ProfileProps> = ({ userId, onBack, onLogout }) => {
  // If userId is provided, fetch that user. If not, fetch my user.
  // We need two different queries or one query that handles both.
  // api.users.get takes userId. api.users.getMyUser takes nothing.

  const user = useQuery(userId ? api.users.get : api.users.getMyUser, userId ? { userId } : {});
  const reviews = useQuery(api.reviews.list, user ? { targetId: user.tokenIdentifier } : "skip");
      ...
      <div className="bg-white p-6 pb-8 shadow-sm relative rounded-b-3xl">
        ...
        <div className="flex flex-col items-center mt-4">
          <div className="relative">
            <img src={user.avatar || "https://via.placeholder.com/150"} alt={user.name} className="w-24 h-24 rounded-full border-4 border-emerald-100 mb-4 object-cover" />
          ...
          <p className="text-gray-500 text-sm mb-4">{user.location || "Terrassa"} • Se unió en 2023</p>
          ...
          <div className="flex items-center gap-6 w-full justify-center border-t border-gray-100 pt-4">
            <div className="text-center">
              ...
              <p className="text-xs text-gray-400">{user.reviewCount || 0} reseñas</p>
            </div>
            <div className="w-px h-8 bg-gray-200"></div>
            <div className="text-center">
              <p className="font-bold text-xl text-gray-900">--</p>
              ...
              <div key={review._id} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center gap-2">
                    <img src={review.reviewerAvatar || "https://via.placeholder.com/32"} alt="" className="w-8 h-8 rounded-full bg-gray-200" />
